...
CACHE["get_vits_phoneme_ids"]["symbols"] = [CACHE["get_vits_phoneme_ids"]["_pad"]] + list(CACHE["get_vits_phoneme_ids"]["_punctuation"]) + list(CACHE["get_vits_phoneme_ids"]["_letters"]) + list(CACHE["get_vits_phoneme_ids"]["_letters_ipa"])
CACHE["get_vits_phoneme_ids"]["_symbol_to_id"] = {s: i for i, s in enumerate(CACHE["get_vits_phoneme_ids"]["symbols"])}



def audiocaps_embedding(config, dl_output, metadata):
    ...
    fname = metadata["wav"]
    wave_name = os.path.basename(fname)[:-4]
    wave_embedding = np.load(f"/mnt/fast/nobackup/scratch4weeks/yy01071/audioldm3/audiocaps_embeddings/all_embeddings/{wave_name}.npy")


    # ipdb.set_trace()
    tensor_float = torch.from_numpy(wave_embedding).float()

    tensor_float = (tensor_float - mean) / std


    # ipdb.set_trace()

    return {"codec_embedding": tensor_float}
...
def mae_retrival(config,dl_output,metadata):
    norm_mean= -4.2677393
    norm_std= 4.5689974
    datum = metadata
    retrival_list = datum["score_list"]
    retrival_list = retrival_list[1:re_num+1]
    ...
    for retrival_name in retrival_list:
        ...
        if(sr != 16000):
            waveform_16k = torchaudio.functional.resample(waveform, orig_freq=sr, new_freq=16000)
        else:
    ...
    fbanks = torch.cat(waves)
    return {"retrival_fbank": fbank}

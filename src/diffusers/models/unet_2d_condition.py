...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    Parameters:
        ...
        global_embeddings_concat (`bool`, *optional*, defaults to `False`): Whether to concatenate the time
    ...
    def __init__(
        ...
        global_embeddings_concat: bool = True,
        ...
        if(global_additional_cond_dim is not None):
            addition_embed_type = "global"
            self.add_embedding = GlobalTimeEmbedding(
                global_additional_cond_dim, time_embed_dim
            )
        ...
        if global_embeddings_concat and global_additional_cond_dim is not None:
        ...
        if(cross_attention_dim is not None):
            ...
            reversed_cross_attention_dim = None
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                resnet_act_fn=act_fn,
                resnet_groups=norm_num_groups,
                cross_attention_dim=reversed_cross_attention_dim[i] if(reversed_cross_attention_dim is not None) else None,
    ...
    def forward(
        ...
        if(self.encoder_hid_proj is not None and self.cross_attention_dim is not None):
        ...
        if self.global_additional_cond_dim is not None:
            aug_emb = self.add_embedding(global_condition)
        ...
        for downsample_block in self.down_blocks:
            # if(encoder_hidden_states is not None): print("down",encoder_hidden_states.size())
            # if(encoder_attention_mask is not None): print("down",encoder_attention_mask.size())
        ...
        if self.mid_block is not None:
            # if(encoder_hidden_states is not None): print("mid",encoder_hidden_states.size())
            # if(encoder_attention_mask is not None): print("mid",encoder_attention_mask.size())
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            # if(encoder_hidden_states is not None): print("up", encoder_hidden_states.size())
            # if(encoder_attention_mask is not None): print("up", encoder_attention_mask.size())

...
class Conv1dBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1d = nn.Conv1d(in_channels, out_channels, kernel_size, padding=padding, stride=stride)
...
def conv_nd(dims, *args, **kwargs):
    ...
    """ 
    global extend_conv1d

    # ipdb.set_trace()
    if dims == 1:
        if extend_conv1d:
            return Conv1dBlock(*args, **kwargs)
        else:   
            return nn.Conv1d(*args, **kwargs)
        # return Conv1dBlock(in_channels, out_channels, kernal_size,*args)   
    elif dims == 2:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            # Conv1dBlock(dims, out_channels, 3)  # 128,128,5
        )
        ...
        if self.out_channels == channels:
            ...
            # self.skip_connection = Conv1dBlock(dims, out_channels, 3)  # 128,128,5
        else:
    ...
    def __init__(
        ...
        if extra_sa_layer:
            middle_layers.append(
                SpatialTransformer(
                    ch, num_heads, dim_head, depth=transformer_depth, context_dim=None,dims=dims
                )
            )
        for context_dim_id in range(len(context_dim)):
            middle_layers.append(
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                # print(f"in level {level} the in channel is 1:{ch} + 2:{ich}")
                layers = [
                    ResBlock(
                        ch + ich,
                        time_embed_dim
                ...
                if level and i == num_res_blocks:
                    # print(f"adding up layer in level {level} and i {i}")
                    out_ch = ch
    ...
    def forward(
        ...
        **kwargs,
        ...
        if context_list==None:
            context_list = []
        if context_attn_mask_list == None:
